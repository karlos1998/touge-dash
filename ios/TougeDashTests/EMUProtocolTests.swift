...
final class EMUProtocolTests: XCTestCase {
    ...
    func testMapsScaledEngineChannels() {
        ...
        accumulator.apply(EMUFrame(channel: 3, rawValue: 200))
        accumulator.apply(EMUFrame(channel: 5, rawValue: 511))
        accumulator.apply(EMUFrame(channel: 8, rawValue: 845))
        accumulator.apply(EMUFrame(channel: 9, rawValue: 862))
        ...
        XCTAssertEqual(accumulator.snapshot.throttlePercent, 100, accuracy: 0.001)
        XCTAssertEqual(accumulator.snapshot.batteryVoltage, 511.0 / 37.0, accuracy: 0.001)
        XCTAssertEqual(accumulator.snapshot.egt1Celsius, 845, accuracy: 0.001)
        XCTAssertEqual(accumulator.snapshot.egt2Celsius, 862, accuracy: 0.001)
