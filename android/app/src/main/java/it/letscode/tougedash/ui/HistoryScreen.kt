...
private fun SessionList(container: AppContainer, select: (String) -> Unit) {
    ...
    Column(Modifier.fillMaxSize()) {
        Text(appText("DRIVE ARCHIVE", "ARCHIWUM PRZEJAZDÓW"), color = MaterialTheme.colorScheme.primary, fontSize = 11.sp, fontWeight = FontWeight.Bold, modifier = Modifier.padding(start = 18.dp, top = 18.dp))
        ...
        if (sessions.isEmpty()) {
            Column(Modifier.fillMaxSize(), horizontalAlignment = Alignment.CenterHorizontally, verticalArrangement = Arrangement.Center) {
                Icon(Icons.Default.DirectionsCar, null, Modifier.size(54.dp), tint = MaterialTheme.colorScheme.onSurfaceVariant)
                Text(appText("No recorded drives yet", "Nie ma jeszcze zapisanych przejazdów"), color = MaterialTheme.colorScheme.onSurfaceVariant, modifier = Modifier.padding(top = 14.dp))
            }
    ...
    deleteSession?.let { session ->
        AlertDialog(
            ...
            text = { Text(appText("Telemetry, incident reports, notes and local source videos assigned to this drive will be removed from this device.", "Telemetria, raporty incydentów, notatki i lokalne filmy źródłowe przypisane do przejazdu zostaną usunięte z urządzenia."), color = MaterialTheme.colorScheme.onSurfaceVariant) },
...
private fun LocalArchiveStorageFooter(storageBytes: Long) {
    Row(
        Modifier
            .fillMaxWidth()
            .background(MaterialTheme.colorScheme.onSurface.copy(alpha = .025f), RoundedCornerShape(8.dp))
        ...
        Text(bytes(storageBytes), color = MaterialTheme.colorScheme.secondary, fontWeight = FontWeight.Black)
    }
}

@Composable
private fun ManualSessionSplitCard(sampleCount: Int, split: () -> Unit) {
    TougePanelSurface(MaterialTheme.colorScheme.primary, Modifier.fillMaxWidth().padding(horizontal = 14.dp, vertical = 12.dp)) {
        Column(Modifier.padding(16.dp)) {
            Row(Modifier.fillMaxWidth(), verticalAlignment = Alignment.CenterVertically) {
                Box(
                    Modifier.size(42.dp).background(MaterialTheme.colorScheme.primary.copy(alpha = .13f), RoundedCornerShape(10.dp)),
                ...
                Box(Modifier.size(8.dp).background(MaterialTheme.colorScheme.secondary, RoundedCornerShape(50)))
            }
            Text(
                appText(
...
private fun SessionRow(session: DriveSessionEntity, tags: List<it.letscode.tougedash.cloud.CloudSyncRepository.DriveTag>, videos: List<VideoProjectEntity>, select: (String) -> Unit, delete: (DriveSessionEntity) -> Unit) {
    val accent = when (session.syncState) {
        SyncState.SYNCED -> MaterialTheme.colorScheme.secondary
        ...
        Column(Modifier.padding(16.dp)) {
            Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween, verticalAlignment = Alignment.CenterVertically) {
                Column {
                    ...
                    if (session.customName != null) Text(DateFormat.getDateTimeInstance(DateFormat.MEDIUM, DateFormat.SHORT).format(Date(session.startedAt)), color = MaterialTheme.colorScheme.onSurfaceVariant, fontSize = 10.sp)
                    Text("${duration(session.endedAt - session.startedAt)}  •  ${session.sampleCount} ${appText("samples", "próbek")}", color = MaterialTheme.colorScheme.onSurfaceVariant, fontSize = 12.sp)
                }
                Row(verticalAlignment = Alignment.CenterVertically) {
                    SyncBadge(session)
                    IconButton(onClick = { delete(session) }) { Icon(Icons.Default.Delete, appText("Delete drive", "Usuń przejazd"), tint = MaterialTheme.colorScheme.error) }
                }
            }
            ...
            if (videos.isNotEmpty()) {
                Row(
                    Modifier.padding(top = 9.dp).background(MaterialTheme.colorScheme.primary.copy(alpha = .1f), RoundedCornerShape(6.dp)).padding(horizontal = 9.dp, vertical = 6.dp),
                    verticalAlignment = Alignment.CenterVertically
                ) {
                    Icon(Icons.Default.Videocam, null, tint = MaterialTheme.colorScheme.primary, modifier = Modifier.size(16.dp))
                    Text(
                        appText(
            ...
            Row(Modifier.fillMaxWidth().padding(top = 14.dp), horizontalArrangement = Arrangement.SpaceBetween) {
                MiniValue(appText("MAX BOOST", "MAX DOŁADOWANIE"), "${"%.2f".format(session.maxBoostBar)} bar", MaterialTheme.colorScheme.primary)
                MiniValue(appText("MAX OIL", "MAX OLEJ"), "${session.maxOilTemperatureCelsius.roundToInt()}°C", MaterialTheme.colorScheme.tertiary)
                MiniValue(appText("MAX COOLANT", "MAX PŁYN"), "${session.maxCoolantCelsius.roundToInt()}°C", MaterialTheme.colorScheme.secondary)
            }
            if (session.syncState == SyncState.UPLOADING || session.syncProgress > 0f && session.syncProgress < 1f) {
                ...
                Text("${(session.syncProgress * 100).roundToInt()}%  •  ${bytes(session.syncBytesSent)} / ${bytes(session.syncBytesTotal)}", color = MaterialTheme.colorScheme.onSurfaceVariant, fontSize = 10.sp)
            }
            session.syncError?.let { Text(it, color = MaterialTheme.colorScheme.error, fontSize = 11.sp, modifier = Modifier.padding(top = 7.dp)) }
        }
    }
}
...
private fun SyncBadge(session: DriveSessionEntity) {
    val (icon, color, text) = when (session.syncState) {
        SyncState.SYNCED -> Triple(Icons.Default.CloudDone, MaterialTheme.colorScheme.secondary, appText("SYNCED", "ZSYNCHRONIZOWANO"))
        SyncState.FAILED -> Triple(Icons.Default.Error, MaterialTheme.colorScheme.error, appText("ERROR", "BŁĄD"))
        SyncState.UPLOADING -> Triple(Icons.Default.CloudDone, MaterialTheme.colorScheme.primary, appText("SYNCING", "SYNCHRONIZACJA"))
        else -> Triple(Icons.Default.CloudOff, MaterialTheme.colorScheme.tertiary, appText("LOCAL", "LOKALNIE"))
    }
...
private fun MiniValue(title: String, value: String, color: Color) {
    Column { Text(title, color = MaterialTheme.colorScheme.onSurfaceVariant, fontSize = 8.sp, fontWeight = FontWeight.Bold); Text(value, color = color, fontWeight = FontWeight.Black) }
}
        ...
        item {
            Row(Modifier.fillMaxWidth().padding(8.dp), verticalAlignment = Alignment.CenterVertically) {
                ...
                Column(Modifier.weight(1f)) {
                    Text(appText("DRIVE REPORT", "RAPORT PRZEJAZDU"), color = MaterialTheme.colorScheme.primary, fontSize = 10.sp, fontWeight = FontWeight.Bold)
                    Text(session?.let { it.customName ?: DateFormat.getDateTimeInstance().format(Date(it.startedAt)) } ?: "…", fontWeight = FontWeight.Black)
                }
                if (session != null) {
                    IconButton(onClick = { showMetadata = true }) { Icon(Icons.Default.Edit, appText("Edit name and tags", "Edytuj nazwę i tagi"), tint = MaterialTheme.colorScheme.primary) }
                    IconButton(onClick = { showShare = true }) { Icon(Icons.Default.Share, appText("Share drive", "Udostępnij przejazd"), tint = MaterialTheme.colorScheme.secondary) }
                }
                IconButton(onClick = { noteDialog = true }) { Icon(Icons.AutoMirrored.Filled.NoteAdd, null, tint = MaterialTheme.colorScheme.secondary) }
            }
        }
        if (session == null) item { Box(Modifier.fillMaxWidth().height(300.dp), contentAlignment = Alignment.Center) { CircularProgressIndicator() } }
            ...
            if (session!!.syncState == SyncState.FAILED) item {
                Column(Modifier.fillMaxWidth().padding(horizontal = 14.dp)) {
                    Text(session!!.syncError ?: appText("Synchronization failed", "Synchronizacja nie powiodła się"), color = MaterialTheme.colorScheme.error, fontSize = 11.sp)
            ...
            if (chartSamples.isEmpty()) item { MissingTelemetryCard(session!!.sampleCount) }
                ...
                item {
                    ...
                    Text(appText("Move the time cursor; the page remains vertically scrollable", "Przesuwaj kursor czasu; stronę nadal możesz przewijać pionowo"), color = MaterialTheme.colorScheme.onSurfaceVariant, fontSize = 10.sp, modifier = Modifier.padding(horizontal = 18.dp))
                }
                ...
                item { TelemetryChart(appText("Boost / oil pressure", "Doładowanie / ciśnienie oleju"), chartSamples, selected, session!!.startedAt, session!!.endedAt, accelerationAttempts, listOf(TelemetryMetric.BOOST to MaterialTheme.colorScheme.primary, TelemetryMetric.OIL_PRESSURE to MaterialTheme.colorScheme.secondary), updateCursor) }
                item { TelemetryChart(appText("Temperatures", "Temperatury"), chartSamples, selected, session!!.startedAt, session!!.endedAt, accelerationAttempts, listOf(TelemetryMetric.OIL_TEMPERATURE to MaterialTheme.colorScheme.tertiary, TelemetryMetric.COOLANT to MaterialTheme.colorScheme.primary), updateCursor) }
                item { TelemetryChart("EGT", chartSamples, selected, session!!.startedAt, session!!.endedAt, accelerationAttempts, listOf(TelemetryMetric.EGT1 to MaterialTheme.colorScheme.tertiary, TelemetryMetric.EGT2 to MaterialTheme.colorScheme.error), updateCursor) }
                item { TelemetryChart(appText("RPM / speed", "Obroty / prędkość"), chartSamples, selected, session!!.startedAt, session!!.endedAt, accelerationAttempts, listOf(TelemetryMetric.RPM to MaterialTheme.colorScheme.error, TelemetryMetric.SPEED to MaterialTheme.colorScheme.secondary), updateCursor) }
            }
...
private fun MissingTelemetryCard(sampleCount: Int) {
    TougePanelSurface(MaterialTheme.colorScheme.tertiary, Modifier.fillMaxWidth().padding(horizontal = 14.dp, vertical = 7.dp)) {
...
private fun SessionSummary(session: DriveSessionEntity) {
    TougePanelSurface(MaterialTheme.colorScheme.primary, Modifier.fillMaxWidth().padding(14.dp)) {
        Column(Modifier.padding(16.dp)) {
            Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween) {
                MiniValue(appText("DURATION", "CZAS"), duration(session.endedAt - session.startedAt), MaterialTheme.colorScheme.primary)
                MiniValue(appText("SAMPLES", "PRÓBKI"), session.sampleCount.toString(), MaterialTheme.colorScheme.secondary)
                MiniValue(appText("DISTANCE", "DYSTANS"), "%.1f km".format(session.distanceMeters / 1000), MaterialTheme.colorScheme.tertiary)
            }
...
private fun TelemetryCursor(sample: TelemetrySampleEntity?, startedAt: Long) {
    Row(Modifier.fillMaxWidth().padding(horizontal = 16.dp), horizontalArrangement = Arrangement.spacedBy(12.dp)) {
        listOf(
            Triple(appText("TIME", "CZAS"), sample?.let { duration(it.recordedAt - startedAt) } ?: "—", MaterialTheme.colorScheme.primary),
            Triple(appText("BOOST", "DOŁADOWANIE"), sample?.let { "%.2f bar".format(it.boostBar) } ?: "—", MaterialTheme.colorScheme.primary),
            Triple(appText("OIL", "OLEJ"), sample?.let { "${it.oilTemperatureCelsius.roundToInt()}°C" } ?: "—", MaterialTheme.colorScheme.tertiary),
            Triple(appText("SPEED", "PRĘDKOŚĆ"), sample?.let { "${it.speedKph.roundToInt()} km/h" } ?: "—", MaterialTheme.colorScheme.secondary)
...
private fun TelemetryChart(
    ...
    val accelerationColors = attempts.associate { it.id to accelerationColor(it.type) }
    val chartGridStrong = MaterialTheme.colorScheme.onSurface.copy(alpha = .09f)
    val chartGridSoft = MaterialTheme.colorScheme.onSurface.copy(alpha = .055f)
    val chartCursor = MaterialTheme.colorScheme.onSurface.copy(alpha = .78f)
    val chartLabel = MaterialTheme.colorScheme.onSurfaceVariant
    val selectAtFraction: (Float) -> Unit = { fraction ->
        selectTimestamp(startedAt + (sessionDuration * fraction.coerceIn(0f, 1f)).toLong())
    }
    TougePanelSurface(series.firstOrNull()?.second ?: MaterialTheme.colorScheme.primary, Modifier.fillMaxWidth().padding(horizontal = 14.dp, vertical = 7.dp)) {
        Column(Modifier.padding(14.dp)) {
            Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween) {
                Text(title.uppercase(), fontSize = 10.sp, fontWeight = FontWeight.Bold)
                Text(series.joinToString("  ") { it.first.localizedName(language) }, color = MaterialTheme.colorScheme.onSurfaceVariant, fontSize = 9.sp)
            }
            Canvas(
                Modifier
                    .fillMaxWidth()
                ...
                val labelPaint = AndroidPaint(AndroidPaint.ANTI_ALIAS_FLAG).apply {
                    color = chartLabel.toArgbWithAlpha(.78f)
                    textSize = 8.dp.toPx()
                ...
                repeat(4) { index ->
                    ...
                    drawLine(chartGridStrong, Offset(left, y), Offset(left + plotWidth, y), 1.dp.toPx())
                ...
                repeat(3) { index ->
                    ...
                    drawLine(chartGridSoft, Offset(x, top), Offset(x, top + plotHeight), 1.dp.toPx())
                    labelPaint.textAlign = when (index) { 0 -> AndroidPaint.Align.LEFT; 2 -> AndroidPaint.Align.RIGHT; else -> AndroidPaint.Align.CENTER }
                    labelPaint.color = chartLabel.toArgbWithAlpha(.76f)
                ...
                attempts.forEach { attempt ->
                    ...
                    drawRect(accelerationColors.getValue(attempt.id).copy(alpha = .12f), topLeft = Offset(startX, top), size = androidx.compose.ui.geometry.Size((endX - startX).coerceAtLeast(2f), plotHeight))
                }
                ...
                selected?.let {
                    ...
                    drawLine(chartCursor, Offset(x, top), Offset(x, top + plotHeight), 1.dp.toPx())
                }
            }
            selected?.let { point ->
                Row(
                    Modifier.fillMaxWidth().padding(top = 4.dp).background(MaterialTheme.colorScheme.onSurface.copy(alpha = .035f), RoundedCornerShape(6.dp)).padding(horizontal = 9.dp, vertical = 8.dp),
                    horizontalArrangement = Arrangement.spacedBy(12.dp),
                    verticalAlignment = Alignment.CenterVertically
                ) {
                    MiniValue(appText("TIME", "CZAS"), duration(point.recordedAt - startedAt), MaterialTheme.colorScheme.primary)
                    series.forEach { (metric, color) ->
...
private fun AccelerationSection(values: List<AccelerationAttemptEntity>, startedAt: Long, select: (AccelerationAttemptEntity) -> Unit) {
    Column(Modifier.padding(horizontal = 14.dp, vertical = 7.dp)) {
        Text(appText("ACCELERATION MEASUREMENTS", "POMIARY PRZYSPIESZENIA"), color = MaterialTheme.colorScheme.primary, fontSize = 10.sp, fontWeight = FontWeight.Bold)
        Text(appText("Tap an attempt to move the telemetry cursor to its start.", "Dotknij próby, aby ustawić kursor telemetrii na jej początku."), color = MaterialTheme.colorScheme.onSurfaceVariant, fontSize = 10.sp)
            ...
            if (attempts.isNotEmpty()) {
                ...
                Row(Modifier.fillMaxWidth().padding(top = 8.dp).background(accelerationColor(type.name).copy(alpha = .09f), RoundedCornerShape(6.dp)).clickable { select(best) }.padding(12.dp), verticalAlignment = Alignment.CenterVertically) {
                    Column(Modifier.weight(1f)) {
                        ...
                        Text("${attempts.size} ${appText("attempts", "prób")} • ${appText("best", "najlepszy")} %.2f s".format(best.durationMillis / 1_000.0), color = MaterialTheme.colorScheme.onSurfaceVariant, fontSize = 10.sp)
                    }
                ...
                Row(
                    ...
                    attempts.sortedBy { it.startedAt }.forEachIndexed { index, attempt ->
                        Text(
                            ...
                            color = if (attempt.id == best.id) Color.Black else MaterialTheme.colorScheme.onSurface,
                            fontSize = 10.sp,
...
private fun accelerationColor(type: String): Color = when (type) {
    AccelerationType.ZERO_TO_100.name -> MaterialTheme.colorScheme.secondary
    AccelerationType.HUNDRED_TO_200.name -> MaterialTheme.colorScheme.primary
    else -> MaterialTheme.colorScheme.tertiary
}
...
private fun IncidentSection(values: List<IncidentEntity>, startedAt: Long, select: (IncidentEntity) -> Unit) {
    Column(Modifier.padding(horizontal = 14.dp, vertical = 7.dp)) {
        Text(appText("INCIDENTS", "INCYDENTY"), color = MaterialTheme.colorScheme.error, fontSize = 10.sp, fontWeight = FontWeight.Bold)
        values.forEach { incident ->
            Row(Modifier.fillMaxWidth().padding(vertical = 5.dp).background(MaterialTheme.colorScheme.error.copy(alpha = .08f), RoundedCornerShape(4.dp)).clickable { select(incident) }.padding(12.dp), verticalAlignment = Alignment.CenterVertically) {
                ...
                Column(Modifier.padding(start = 10.dp).weight(1f)) {
                    ...
                    Text("${duration(incident.triggeredAt - startedAt)} • ${incident.triggerValue} ${incident.triggerUnit}", color = MaterialTheme.colorScheme.onSurfaceVariant, fontSize = 11.sp)
                    ...
                    incident.syncError?.let { Text(it, color = MaterialTheme.colorScheme.error, fontSize = 9.sp) }
                }
                Column(horizontalAlignment = Alignment.End) {
                    Text(if (incident.severity == "CRITICAL") appText("CRITICAL", "KRYTYCZNY") else appText("WARNING", "OSTRZEŻENIE"), color = MaterialTheme.colorScheme.error, fontSize = 9.sp, fontWeight = FontWeight.Bold)
                    Text(if (incident.syncState == SyncState.SYNCED) appText("CLOUD", "CHMURA") else appText("LOCAL", "LOKALNIE"), color = if (incident.syncState == SyncState.SYNCED) MaterialTheme.colorScheme.secondary else MaterialTheme.colorScheme.tertiary, fontSize = 8.sp, fontWeight = FontWeight.Black)
                }
            }
        }
    }
...
private fun IncidentReportDialog(incident: IncidentEntity, container: AppContainer, dismiss: () -> Unit) {
    ...
    AlertDialog(
        ...
        title = {
            Column {
                Text(appText("INCIDENT REPORT", "RAPORT INCYDENTU"), color = MaterialTheme.colorScheme.error, fontSize = 10.sp, fontWeight = FontWeight.Bold)
        ...
        text = {
            Column(Modifier.heightIn(max = 620.dp).verticalScroll(rememberScrollState()), verticalArrangement = Arrangement.spacedBy(12.dp)) {
                Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween) {
                    MiniValue(appText("TRIGGER", "WARTOŚĆ"), "${incident.triggerValue} ${incident.triggerUnit}", MaterialTheme.colorScheme.error)
                    MiniValue(appText("LIMIT", "PRÓG"), "${incident.thresholdValue} ${incident.triggerUnit}", MaterialTheme.colorScheme.tertiary)
                    MiniValue("RPM", incident.triggerRpm.roundToInt().toString(), MaterialTheme.colorScheme.primary)
                }
                Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween) {
                    MiniValue(appText("DURATION", "CZAS TRWANIA"), duration(incident.conditionDurationMillis), MaterialTheme.colorScheme.error)
                    MiniValue(appText("FUEL PRESSURE", "CIŚNIENIE PALIWA"), "${"%.2f".format(incident.triggerFuelPressureBar)} bar", MaterialTheme.colorScheme.secondary)
                }
                Text(
                    appText(
                ...
                Text(appText("SHARE WITH A MECHANIC", "WYŚLIJ MECHANIKOWI"), color = MaterialTheme.colorScheme.primary, fontSize = 10.sp, fontWeight = FontWeight.Bold)
                Text(appText("The link opens only this report and does not require an account.", "Link otwiera wyłącznie ten raport i nie wymaga konta."), color = MaterialTheme.colorScheme.onSurfaceVariant, fontSize = 11.sp)
                ...
                if (shareUrl == null) Button(
                    ...
                    Text(shareUrl!!, color = MaterialTheme.colorScheme.secondary, fontSize = 10.sp)
                    Button(onClick = {
                ...
                if (!container.authRepository.isAuthenticated) Text(appText("Sign in to Touge Dash Cloud before creating a link.", "Zaloguj się do Touge Dash Cloud, aby utworzyć link."), color = MaterialTheme.colorScheme.tertiary, fontSize = 11.sp)
                error?.let { Text(it, color = MaterialTheme.colorScheme.error, fontSize = 11.sp) }
                Text(appText("NOTE AT THE TRIGGER", "NOTATKA PRZY ZDARZENIU"), color = MaterialTheme.colorScheme.secondary, fontSize = 10.sp, fontWeight = FontWeight.Bold)
...
private fun IncidentSparkline(points: List<CapturedTelemetryPoint>) {
    val boostColor = MaterialTheme.colorScheme.primary
    val oilColor = MaterialTheme.colorScheme.secondary
    val coolantColor = MaterialTheme.colorScheme.tertiary
    val triggerColor = MaterialTheme.colorScheme.error
    Card(colors = CardDefaults.cardColors(containerColor = MaterialTheme.colorScheme.onSurface.copy(alpha = .045f))) {
        Column(Modifier.padding(10.dp)) {
            Text(appText("BOOST • OIL PRESSURE • COOLANT", "BOOST • CIŚNIENIE OLEJU • PŁYN"), color = MaterialTheme.colorScheme.onSurfaceVariant, fontSize = 8.sp, fontWeight = FontWeight.Bold)
            Canvas(Modifier.fillMaxWidth().height(130.dp).padding(top = 8.dp)) {
                if (points.size < 2) return@Canvas
                val series = listOf(
                    points.map { it.boostBar } to boostColor,
                    points.map { it.oilPressureBar } to oilColor,
                    points.map { it.coolantCelsius } to coolantColor
                )
                ...
                triggerX?.let { drawLine(triggerColor, Offset(it, 0f), Offset(it, size.height), 1.dp.toPx()) }
            }
        }
    }
}
...
private fun AnnotationSection(values: List<AnnotationEntity>, startedAt: Long) {
    Column(Modifier.padding(14.dp)) {
        Text(appText("NOTES", "NOTATKI"), color = MaterialTheme.colorScheme.secondary, fontSize = 10.sp, fontWeight = FontWeight.Bold)
...
private fun DriveTagChip(tag: it.letscode.tougedash.cloud.CloudSyncRepository.DriveTag) {
    ...
    val color = remember(tag.color, fallbackColor) {
        runCatching { Color(android.graphics.Color.parseColor(tag.color)) }.getOrDefault(fallbackColor)
    }
    Row(
...
private fun DriveMetadataDialog(container: AppContainer, session: DriveSessionEntity, dismiss: () -> Unit) {
    ...
    AlertDialog(
        onDismissRequest = dismiss,
        title = { Column { Text(appText("ORGANIZE HISTORY", "ORGANIZACJA HISTORII"), color = MaterialTheme.colorScheme.primary, fontSize = 10.sp, fontWeight = FontWeight.Black); Text(appText("Drive name and tags", "Nazwa i tagi przejazdu"), fontWeight = FontWeight.Black) } },
        text = {
            Column(Modifier.heightIn(max = 560.dp).verticalScroll(rememberScrollState()), verticalArrangement = Arrangement.spacedBy(12.dp)) {
                ...
                Text(appText("DRIVE TAGS", "TAGI PRZEJAZDU"), color = MaterialTheme.colorScheme.secondary, fontSize = 10.sp, fontWeight = FontWeight.Black)
                if (tags.isEmpty()) Text(if (container.authRepository.isAuthenticated) appText("No tags yet. Create the first one below.", "Nie ma jeszcze tagów. Utwórz pierwszy poniżej.") else appText("Sign in to manage reusable tags.", "Zaloguj się, aby zarządzać tagami."), color = MaterialTheme.colorScheme.onSurfaceVariant, fontSize = 11.sp)
                tags.forEach { tag ->
                    FilterChip(
                ...
                if (container.authRepository.isAuthenticated) {
                    Text(appText("NEW TAG", "NOWY TAG"), color = MaterialTheme.colorScheme.primary, fontSize = 10.sp, fontWeight = FontWeight.Black)
...
private fun DriveShareDialog(container: AppContainer, session: DriveSessionEntity, samples: List<TelemetrySampleEntity>, dismiss: () -> Unit) {
    ...
    AlertDialog(
        onDismissRequest = dismiss,
        title = { Column { Text(appText("SHARE A DRIVE", "UDOSTĘPNIJ PRZEJAZD"), color = MaterialTheme.colorScheme.primary, fontSize = 10.sp, fontWeight = FontWeight.Black); Text(session.customName ?: DateFormat.getDateTimeInstance().format(Date(session.startedAt)), fontWeight = FontWeight.Black) } },
        text = {
            Column(Modifier.heightIn(max = 580.dp).verticalScroll(rememberScrollState()), verticalArrangement = Arrangement.spacedBy(11.dp)) {
                ...
                if (fragment) {
                    Text(appText("Choose the beginning and end like when trimming a video.", "Wybierz początek i koniec tak jak przy przycinaniu filmu."), color = MaterialTheme.colorScheme.onSurfaceVariant, fontSize = 11.sp)
                    Text("${duration((startSeconds * 1000).toLong())}  →  ${duration((endSeconds * 1000).toLong())}", color = MaterialTheme.colorScheme.secondary, fontWeight = FontWeight.Black)
                ...
                if (previewSamples.isNotEmpty()) {
                    Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween) {
                        MiniValue(appText("SAMPLES", "PRÓBKI"), previewSamples.size.toString(), MaterialTheme.colorScheme.primary)
                        MiniValue(appText("MAX BOOST", "MAX DOŁADOWANIE"), "%.2f bar".format(previewSamples.maxOf { it.boostBar }), MaterialTheme.colorScheme.secondary)
                        MiniValue(appText("MAX SPEED", "MAX PRĘDKOŚĆ"), "${previewSamples.maxOf { it.speedKph }.roundToInt()} km/h", MaterialTheme.colorScheme.tertiary)
                    }
                    Text(
                        appText(
                ...
                Text(appText("LINK EXPIRATION", "WAŻNOŚĆ LINKU"), color = MaterialTheme.colorScheme.primary, fontSize = 10.sp, fontWeight = FontWeight.Black)
                Row(horizontalArrangement = Arrangement.spacedBy(6.dp)) {
                    FilterChip(unit == "HOURS", { unit = "HOURS"; amount = amount.coerceIn(1, 168) }, label = { Text(appText("Hours", "Godziny")) })
                    FilterChip(unit == "DAYS", { unit = "DAYS"; amount = amount.coerceIn(1, 365) }, label = { Text(appText("Days", "Dni")) })
                    FilterChip(unit == "FOREVER", { unit = "FOREVER" }, label = { Text(appText("Forever", "Bezterminowo")) })
                ...
                if (links.isNotEmpty()) {
                    Text(appText("ACTIVE LINKS", "AKTYWNE LINKI"), color = MaterialTheme.colorScheme.primary, fontSize = 10.sp, fontWeight = FontWeight.Black)
                    links.forEach { link ->
                        Row(
                            Modifier.fillMaxWidth().background(MaterialTheme.colorScheme.onSurface.copy(alpha = .035f), RoundedCornerShape(7.dp)).padding(10.dp),
                            verticalAlignment = Alignment.CenterVertically
                        ) {
                            Column(Modifier.weight(1f)) {
                                Text(link.expiresAt ?: appText("No expiration", "Bezterminowo"), fontSize = 10.sp, fontWeight = FontWeight.Bold)
                                link.range?.let { Text("${duration(it.startOffsetMillis)} → ${duration(it.endOffsetMillis)}", color = MaterialTheme.colorScheme.secondary, fontSize = 9.sp) }
                            }
                            IconButton(onClick = {
                                scope.launch {
                                    working = true; error = null
                            ...
                            }) { Icon(Icons.Default.Delete, appText("Revoke link", "Unieważnij link"), tint = MaterialTheme.colorScheme.error) }
                        }
                    }
                }
                if (!container.authRepository.isAuthenticated) Text(appText("Sign in to Touge Dash Cloud before creating a link.", "Zaloguj się do Touge Dash Cloud, aby utworzyć link."), color = MaterialTheme.colorScheme.tertiary, fontSize = 11.sp)
...
private fun RouteMap(samples: List<TelemetrySampleEntity>) {
    ...
    Card(Modifier.fillMaxWidth().padding(14.dp), colors = CardDefaults.cardColors(containerColor = MaterialTheme.colorScheme.surface)) {
        Column {
            Row(Modifier.padding(14.dp), verticalAlignment = Alignment.CenterVertically) { Icon(Icons.Default.Map, null, tint = MaterialTheme.colorScheme.primary); Text(appText(" ROUTE", " TRASA"), fontWeight = FontWeight.Bold) }
            AndroidView(
                factory = {
